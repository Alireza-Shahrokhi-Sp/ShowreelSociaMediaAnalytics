...
Outputs:
  - outputs/stage2_persona_20_percent/stage2_responses_sample.jsonl   (resumable checkpoint; raw model text per group)
  - outputs/stage2_persona_20_percent/user_personas_sample.parquet    (final joined table)

Usage:
  python run_stage2_async_sample.py                 # classify 20% users
  python run_stage2_async_sample.py --max-users 30  # smoke test (1 call ~= 10 users)
  python run_stage2_async_sample.py --concurrency 100 --users-per-request 10
  python run_stage2_async_sample.py --dry-run       # build one request, print it, no API call
...
OUT_DIR = "outputs/stage2_persona_20_percent"
CHECKPOINT_PATH = f"{OUT_DIR}/stage2_responses_sample.jsonl"
RESULTS_PATH = f"{OUT_DIR}/user_personas_sample.parquet"
...
async def run(args):
    ...
    # NEW: Sample 20% of the users for this run
    ...
    print(f"\n{'=' * 64}\nSTAGE 2 (async) - Deterministic Classification (20% Sample)\n"
...
def main():
    ap = argparse.ArgumentParser(description="Async Stage 2 persona classification (20% Sample).")
